...
def getDateSpans(raw_txt):
    spans = []
    pattern = r'(((mon|tues?|wed(nes?)|thr(us)?|fri|sat(ur)?|sun)(day)?)[,\s]*((jan|febr?)(uary)?|mar(ch)?|apr(il)?|may|june?|july?|aug(ust)?|sept?(ember)?|oct(ober)?|nov(ember)?|dec(ember)?)[,\s]?((0[1-9]|[12]\d|3[01])|([1-9]|[12]\d|3[01]))[,\s]*[12][0-9]{3})|(((mon|tues?|wed(nes?)|thr(us)?|fri|sat(ur)?|sun)(day)?)[,\s]*((jan|febr?)(uary)?|mar(ch)?|apr(il)?|may|june?|july?|aug(ust)?|sept?(ember)?|oct(ober)?|nov(ember)?|dec(ember)?)[,\s]?((0[1-9]|[12]\d|3[01])|([1-9]|[12]\d|3[01])))'
    indexes = [(i.start(),i.end()) for i in re.finditer(pattern,raw_txt,flags=re.IGNORECASE)]
    for t in indexes:
